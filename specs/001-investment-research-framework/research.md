...
**2026-09-22 用户覆盖（追加，不废止原 Serenity 决策）**：供应链顺序为：当前系统广度优先前沿（步骤／组件／构成材料、经手公司、角色、同一步竞争者；命名组件不是不透明叶子）→ 量化下游需求 → 一次短缺／瓶颈检验 → 仅将通过该检验的节点入队并用同一 BFS 再拆（队列为冻结文件）→ 程序估值对照报价判断是否已定价 → 程序估值。单票须有定量路径（经营驱动＋`compute valuation`＋报价对照）。面向用户的完成是多阶段、长程（量级约 10+ agent-hours）的冻结产物交接，不是单次专员步骤。原句“早期线索不强求成熟财务模型”仍适用于运行中途的线索文件，不得作为面向用户的完成出口；“有限地图＋估值不适用”不得结束被请求的分析。用户举例推理服务器→光模块经检验短缺→再拆光模块（Serenity 路径）仅为方法动机，不设收益倍数、不强制具体证券。不新建调度器、图数据库或状态机。
